...
if cfg.Guild == true then
	...
	local function Update(self, event, ...)	
		...
		if IsInGuild() then
			...
			Text:SetText(format(cfg.ColorClass and infoL["Guild"]..": "..init.Colored.."%d" or infoL["Guild"]..": %d", online))
		else
			Text:SetText(cfg.ColorClass and init.Colored..infoL["No Guild"] or infoL["No Guild"])
		end
	end
	...
	Stat:SetScript("OnEnter", function(self)
		...
		GameTooltip:AddDoubleLine(format("<%s>", guildName), format(format("%s: %d/%d", GUILD_ONLINE_LABEL, online, total)),0,.6,1,0,.6,1)
		...
		if standingID ~= 8 then -- Not Max Rep
			...
			barMin = 0
			GameTooltip:AddLine(format(format(RGBToHex(ttsubh.r, ttsubh.g, ttsubh.b), "%s:|r |cFFFFFFFF%s/%s (%s%%)", COMBAT_FACTION_CHANGE, ShortValue(barValue), ShortValue(barMax), ceil((barValue / barMax) * 100))))
		end
		
		local zonec, classc, levelc, info
		local num
		if #guildTable > 45 then
			num = 45
		else
			num = #guildTable
		end		
		for i = 1, num do
			...
			if IsShiftKeyDown() then
				...
				if info[5] ~= "" then GameTooltip:AddLine(format(format("|cff999999%s:|r %s", LABEL_NOTE, info[5])), ttsubh.r, ttsubh.g, ttsubh.b, 1) end
				if info[6] ~= "" then GameTooltip:AddLine(format(format("|cff999999%s:|r %s", GUILD_RANK1_DESC, info[6])), ttoff.r, ttoff.g, ttoff.b, 1) end
			else
		...
		if #guildTable > 45 then
			...
			GameTooltip:AddDoubleLine(infoL["Excessive"],"",.6,.8,1,.6,.8,1)
		end
		GameTooltip:AddDoubleLine(" ","--------------",1,1,1,0.5,0.5,0.5)
